...
import hashlib
import requests
from cmdb_Client.src import plugins
from cmdb_Client.lib.serialize import Json
from cmdb_Client.lib.log import Logger
...
class AutoBase(object):
    ...
    def get_asset(self):
        ...
        try:
            ...
            response = e
        print(' 2.1,获取未收集的主机信息',response.json())
    ...
    def post_asset(self, msg, callback=None):
        ...
        :return:
        """
        print('9.client.post_asset post方式向接口提交资产信息')
        print(msg)
        status = True
        try:
            headers = {}
            ...
            print('post_asset--->response',response)
        except Exception as e:
            response = e
            status = False
        if callback:
...
class AutoAgent(AutoBase):
    ...
    def process(self):
        ...
        :return:
        """
        print('2.client.AutoAgent')

        server_info = plugins.get_server_info()

        print('7.继续client.AutoAgent')
        ...
        print('8.client.AutoAgent完毕! 把主机json数据 发给 client.post_asset')
...
class AutoSSH(AutoBase):
    ...
    def run(self, hostname):
        ...
        server_json = Json.dumps(server_info.data)
        print(' 2.2 run() server_info,server_json',server_info,server_json)
        self.post_asset(server_json, self.callback)
