...
from cmdb.utils import agorithm
...
class HandleNic(object):
    ...
    def process(server_obj, server_info, user_obj):
        ...
        try:
            ...
            print('nic--最新网卡信息:%s,数据库网卡信息:%s, 并集update_list:%s'%(client_nic_dict.keys(),nic_name_list,update_list))
            # ==> 要添加、更新、删除
...
class HandleDisk(object):
    ...
    def process(server_obj, server_info, user_obj):
        ...
        try:
            ...
            print('disk_info',disk_info)####################### print #######################
